...
export const ResultTripItem = ({ trip, onAccept, className }: TripItemProps) => {
    return (
        <div className={cn("flex flex-col items-start px-[22px] py-4", className)}>
            ...
            <div className="flex justify-between items-center w-full">
                ...
                {/* Remove button */}
                <button
                    className="w-[30px] h-[30px] hover:bg-green-50"
                    onClick={() => onAccept?.(trip.id)}
                >
                    <CheckIcon className="w-7 h-6 text-[#16a130] stroke-[4]" />
                </button>
            </div>
        </div>
    );
